...
func CreateTaskStatus(c *fiber.Ctx) error {
	//creating a context
	...
	if err := c.BodyParser(&taskstatus); err != nil {
	...
	//saving data in db
	result, err := service.CreateTaskStatus(ctx, taskstatus)
	if err != nil {
		return c.Status(http.StatusBadRequest).JSON(response.ErrorResponse{
			ApiPath:      c.OriginalURL(),
			ErrorCode:    http.StatusBadRequest,
...
func GetTaskStatusById(c *fiber.Ctx) error {
	//creating a context
	ctx, cancel := context.WithTimeout(context.Background(), time.Duration(database.ContextTime)*time.Second)
	defer cancel()
	...
	//fetch data from DB
	result, err := service.GetTaskStatusByID(ctx, idParam)
	if err != nil {
		return c.Status(http.StatusBadRequest).JSON(response.ErrorResponse{
			ApiPath:      c.OriginalURL(),
			ErrorCode:    http.StatusBadRequest,
...
func GetAllTaskStatus(c *fiber.Ctx) error {
	//creating a context
	ctx, cancel := context.WithTimeout(context.Background(), time.Duration(database.ContextTime)*time.Second)
	defer cancel()
	...
	//fetch data from DB
	result, count, err := service.GetAllTaskStatus(ctx, limit, offset)
	if err != nil {
		return c.Status(http.StatusBadRequest).JSON(response.ErrorResponse{
			ApiPath:      c.OriginalURL(),
			ErrorCode:    http.StatusBadRequest,
...
func UpdateTaskStatus(c *fiber.Ctx) error {
	...
	//update data from DB
	result, err := service.UpdateTaskStatus(ctx, idParam, taskstatus)
	if err != nil {
		return c.Status(http.StatusBadRequest).JSON(response.ErrorResponse{
			ApiPath:      c.OriginalURL(),
			ErrorCode:    http.StatusBadRequest,
...
func DeleteTaskStatus(c *fiber.Ctx) error {
	//creating a context
	ctx, cancel := context.WithTimeout(context.Background(), time.Duration(database.ContextTime)*time.Second)
	defer cancel()
	...
	//delete data from DB
	result, err := service.DeleteTaskStatus(ctx, idParam)
	if err != nil {
		return c.Status(http.StatusBadRequest).JSON(response.ErrorResponse{
			ApiPath:      c.OriginalURL(),
			ErrorCode:    http.StatusBadRequest,
